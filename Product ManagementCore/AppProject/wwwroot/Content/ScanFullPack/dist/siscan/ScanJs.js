    ...
    $.fn.extend({
        scanApp: function (options) {
            var defaults = {
                ...
                ScanedFilePaths: '/Uploads/ajaxUpload/',
                FolderName: '',
                HoSoId: null,
                TwPixelType: 'Gray',
                DPI: 200,
                UrlFileSetup: '/Content/ScanFullPack/FileSetup/Scan_SetUp_SP.zip',
                OCREnabled: false,
                WordEnabled: false,
                ExcelEnabled: false,
                ClearBG: false,
            ...
            this.each(function () {
                ...
                //$boxscandevice.append('<div><label>Nhập tiêu đề</label></div>');
                //$boxscandevice.append('<input class=" form-control" placeholder="Nhập tên hiển thị sau khi Scan" type="text" name="TitleSC" id="TitleSC" />');
                ...
                $this.on('click', '.btn-scan', function () {
                    ...
                    $btnScan = $(this);
                    $message.html('');
                    console.log(window.location.origin + o.UrlUploadHandler);
                    var data = {
                        ...
                        TwPixelType: (o.ExportPixelTypeUI ? $this.find('input[name="PixelType"]:checked').data('value') : o.TwPixelType),
                        DPI: $this.find('.select-dpi').val(),
                        TitleSC: $this.find('input[name="TitleSC"]').val(),
                        DuplexEnabled: $this.find("input[name='Duplex']").is(":checked"),
                        FeederEnable: $this.find("input[name='ADF']").is(":checked"),
                        ShowUI: (o.ExportShowUI ? $this.find("input[name='ShowUI']").is(":checked") : o.ShowUI),
                        ShowProgress: o.ShowProgress,
                    ...
                    $.getJSON(o.Url + 'ScanGo?callback=?', data).done(function (result) {
                        ...
                        if (result.Error) {
                            ...
                            if (o.TitleSC != null && o.TitleSC != '')
                                dataSuccessHandler.TitleSC = o.TitleSC;
